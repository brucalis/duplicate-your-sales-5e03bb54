...
const traditionalItems = [
  ...
  "Interrupções durante o desenvolvimento",
  "Necessidade de esperar a renovação",
  "Fluxo de trabalho limitado",
  "Menos recursos extras",
  "Experiência mais restrita",
];

const superLovableItems = [
  "Crie sem se preocupar com limites",
...
function ComparisonSection() {
  return (
    <Section 
      title="Compare e escolha a melhor experiência"
      subtitle="Veja a diferença entre usar a Super Lovable e depender da experiência tradicional."
    >
      <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-2 lg:gap-12">
        {/* Card Tradicional */}
        <Reveal delay={100}>
          <div className="glass-strong group relative flex h-full flex-col overflow-hidden rounded-[2rem] border-destructive/20 bg-gradient-to-b from-destructive/[0.08] to-transparent p-8 transition-all duration-500 hover:-translate-y-2 hover:border-destructive/40 hover:shadow-[0_20px_40px_-20px_rgba(239,68,68,0.25)]">
            <div className="mb-8 flex items-center gap-3">
              <span className="text-2xl" role="img" aria-label="emoji">😞</span>
              <h3 className="text-xl font-bold tracking-tight text-white/90">Lovable tradicional</h3>
            </div>
            <ul className="space-y-5">
              {traditionalItems.map((item, i) => (
                <li key={i} className="flex items-center gap-3.5 text-[15px] text-muted-foreground">
        ...
        {/* Card Super Lovable */}
        <Reveal delay={200}>
          <div className="glass-strong group relative flex h-full flex-col overflow-hidden rounded-[2rem] border-primary/30 bg-gradient-to-b from-primary/[0.08] to-transparent p-8 shadow-[0_0_50px_-12px_rgba(255,45,187,0.15)] transition-all duration-500 hover:-translate-y-2 hover:border-primary/50 hover:shadow-[0_20px_50px_-20px_rgba(255,45,187,0.3)]">
            {/* Brilho discreto */}
            <div className="pointer-events-none absolute -right-20 -top-20 h-40 w-40 rounded-full bg-primary/20 blur-[60px] transition-opacity duration-500 group-hover:opacity-100" />
            
            <div className="mb-8 flex items-center gap-3">
              <span className="text-2xl" role="img" aria-label="emoji">🚀</span>
              <h3 className="text-xl font-bold tracking-tight text-white/90">Super Lovable</h3>
            </div>
            <ul className="space-y-5">
              {superLovableItems.map((item, i) => (
                <li key={i} className="flex items-center gap-3.5 text-[15px] text-foreground/90">
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-primary/20 transition-transform duration-300 group-hover:scale-110">
                    <Check className="h-3.5 w-3.5 animate-in fade-in zoom-in duration-500 text-primary" strokeWidth={3} />
                  </span>
                  {item}
                </li>
              ))}
      ...
      <Reveal delay={300}>
        <div className="mt-14 text-center">
          <CtaButton href="#planos" size="lg">
            Quero criar sem limites <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
          </CtaButton>
        </div>
      </Reveal>
    </Section>
